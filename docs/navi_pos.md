...
pose:
  position:
    x: 2.5100831985473633
    y: -0.9729030132293701
    z: 0.0
  orientation:
    x: 0.0
    y: 0.0
    z: 0.9999991407696093
    w: 0.0013109004703446361
---
...
pose:
  position:
    x: 1.0242657661437988
    y: -1.0265377759933472
    z: 0.0
  orientation:
    x: 0.0
    y: 0.0
    z: 0.9999860584511261
    w: 0.005280426439306799
---
...
$0.08\,\text{m}$ 间距。本次候选全局路径共得到 38 个非等距采样点。这些点是同一条
`nav_msgs/Path` 的表示点，不应再分别当作 38 个独立 `MoveBaseGoal`。
...
- 原始 seq 20 的 $y=-0.9729\,\text{m}$ 会在转弯时接触底部 `Wall_154`；现调整为
  $y=-0.9000\,\text{m}$，复测后该接触已消失。
- 全流程物理墙体多边形相交采样数：0。
